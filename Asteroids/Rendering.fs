...
module Rendering = 
    ...
    let drawShip (gd:GraphicsDevice) scale = 
        ...
        let iter f pts (vertex: VertexPositionColor[]) = 
            List.iteri (fun i (x, y) -> 
                let x', y' = f x y
                vertex.[i].Position <- Vector3(float32 x', float32 y', 0.f)) pts
    ...
    let drawMeteor (gd:GraphicsDevice)  = 
        ...
        let random_vert _ = Graphics.VertexPositionColor(Vector3(0.f, 0.f, 0.f), Color.White)
        let vertex = Array.init n_verts random_vert
        let iter f size = List.iteri (fun i (Vector(x, y)) ->
                                        let x', y' = f x y
    ...
    let drawShield (gd:GraphicsDevice) = 
        let angles = [for i in 1 .. 31 -> Math.PI * 2.0 * float i / 30.0]
        ...
        let iter f = List.iteri (fun i (Vector(x, y)) ->
            let x', y' = f x y
            vertex.[i/2].Position <- Vector3(float32 x', float32 y', 0.f))
    ...
    let drawBullet (gd:GraphicsDevice) = 
        ...
        let vertex = Array.init n_verts random_vert
        let draw (gd:GraphicsDevice) x y =
             vertex.[0].Position <- Vector3(float32 x, float32 y, 0.f)
             vertex.[1].Position <- Vector3(float32 x + 0.001f, float32 y + 0.001f , 0.f)
