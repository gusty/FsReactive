...
module Game =
    ...
    let hitCorner corner pPrev pCur radius =
        ...
        if not (inCircle corner pPrev radius) && (inCircle corner pCur radius) then
            let dx = Vector.norm (pPrev - corner)
            let dy = rot90AntiClockwise dx
            let adaptVel v =   
                let r = dx * -Vector.dot dx v + dy * Vector.dot v dy
                printf "corner = %A %A\n"  dx dy
                r
    ...
    let crossBox pll plr pul pur xp x radius =
        ...
        match List.filter isSome preds with
        | []   -> None
        | h::t -> h
    ...
    let mkHits ballRadius bricks pBallOption ballOption  = 
        ...
        | Some pBall, Some ball ->
            ...
            let r = List.map (fun (Brick (id, pos)) -> match proc pos with Some f -> Some (id, f) | None -> None) bricks
            catOption r
        | _ -> []
    ...
    let mkVelocity v0 hitsB ballB xPaddleB ballRadius = 
        ...
        let hitWallx (Vector (x, y))  = 
            if x <= -1.0 || x >= 1.0 
            then Some (fun v -> (Vector.rot v (randRange -0.01 0.01)) |> invX)
            else None
        let hitWally (Vector (x, y)) xPaddle = 
            if (y-ballRadius <= paddleY && xPaddle-paddleHalfLength <= x && x <= xPaddle+paddleHalfLength) || y>= 1.0
            then Some (fun v -> (Vector.rot v (randRange -0.01 0.01)) |> invY)
            else None
        let hitBricks hits =
    ...
    let mkPaddle x0 (game:Game) =
        ...
        let mousePosXB = pureB (fun x -> 
                                    if   x - paddleHalfLength < -1.0 
                                    then  -1.0
    ...
    let rec game (theGame:Game) = 
        let noGame = {   
            ball    = Some (Vector( 0.0, 0.0))
            bricks  = []
            xpaddle = 0.0
            nbBalls = 3 }
