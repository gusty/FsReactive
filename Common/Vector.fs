...
module Vector =
    ...
    type  Vector = Vector of (float * float) with 
        ...
        static member length (Vector (xa,ya)) = sqrt (xa*xa+ya*ya)
        static member norm (v:Vector) = v / (Vector.length v)
        static member rot (Vector (x,y)) angle = let cosa, sina = cos angle, sin angle in Vector (x * cosa - y * sina, x * sina + y * cosa)
